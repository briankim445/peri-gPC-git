...
Nrace = 2*Nstraight+2*Ncurve+6*length(gamma);         % number of points making up the racetrack part
...
for i = ceil(Ncurve/2)+ceil(Nstraight/2)+1:Ncurve+ceil(Nstraight/2)
    ...
    y_race(1,i) = centery+0.1*sin(theta);
...
for i = Ncurve+ceil(Ncurve/2)+Nstraight+1:2*Ncurve+Nstraight
    ...
    y_race(1,i) = centery+0.2*sin(theta);
...
%for i = Ncurve+ceil(Ncurve/2)+ceil(Nstraight/2)+1:Ncurve+ceil(Ncurve/2)+Nstraight
 for i = 2*Ncurve+Nstraight+1:2*Ncurve+Nstraight+ceil(Nstraight/2)
    ...
    y_race(1,i) = centery+R1+0.05*(-cos(theta))*cos(theta1)-0.05;
    x_race(1,i) = centerx2-(i-(2*Ncurve+Nstraight)-1)*ds;
    fprintf(vertex_fid, '%1.16e %1.16e\n', x_race(1,i), y_race(1,i));
end
